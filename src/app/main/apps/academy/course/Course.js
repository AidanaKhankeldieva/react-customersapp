						...
						<FuseScrollbars className="w-full overflow-auto">
							...
							>
								{course.steps.map((step, index) => {
                                        let classList = 'flex justify-center';
                                        let classListPaper = 'w-full max-w-lg px-16 overflow-hidden';
                                        if (step.type === 'slide') {
                                            // If slide then make full width
                                          classList = 'flex justify-center';
                                          classListPaper = 'w-full px-16  overflow-hidden';
                                        }
                                        return (
                                          <div className={classList} key={step.id}>
                                              <Paper className={classListPaper} elevation={1}>
							...
							<div className="flex justify-between w-full max-w-xl px-8">
								...
								<div>
									{activeStep < course.steps.length ? (
										...
										<Fab className={classes.successFab} to="/courses" component={Link}>
											<Icon>check</Icon>
										</Fab>
									)}
								</div>
